...
(function (root) {
  ...
  function migrate(raw) {
    ...
    if (typeof raw.profile === 'string') s.profile = raw.profile;
    if (isObj(raw.weekObjectives)) s.weekObjectives = raw.weekObjectives;
    if (isObj(raw.tests)) s.tests = raw.tests;
    if ([0, 1, 2].includes(raw.restDays)) s.restDays = raw.restDays;
    return s;
  }
  ...
  function load(storage) {
    ...
    try {
      ...
      const migrated = isObj(parsed) && !(parsed.version >= SCHEMA_VERSION);
      return { state: migrate(parsed), recovered: false, migrated };
    } catch (e) {
  ...
  const api = { DONE_V6_TO_V7, KEY, BACKUP_KEY, SCHEMA_VERSION, esc, defaults, cleanLog, migrate, load, save, parseImport, slug, exId };
